...
//Reducers
import { getCompensationBoxAllService , deleteCompensationBoxService  } from "../../../store/action/compensationBoxAction";
...
function List() {
  ...
  const dataList = useSelector((store) => store.compesationBoxReducer);
  ...
  const alertDelete = async (id) => {
    let response = await dispatch(deleteCompensationBoxService(id));
  ...
  return (
    <div className='list-compensationBox-main'>
      { view.list === true ?
        <>
          <div className='list-compensationBox-main-button'>
            <button onClick={handleCreate} type="button" className="list-compensationBox-button-title btn btn-primary">Crear</button>
          </div> 
          <DefaultTable data={dataList.data} nms={"compensationBox"} deleteId={deleteInfo} updateId={updateInfo} />
        </>
        : view.create === true ?
