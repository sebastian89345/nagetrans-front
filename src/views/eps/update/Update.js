...
//Reducers
import { updateEpsService } from "../../../store/action/epsAction";
...
function Update({ infoUpdate,setView,getAll }) {
  ...
  const edit = async () => {
    ...
    if(validate === true) { 
        ...
        let response = await dispatch(updateEpsService(body));
  ...
  return (
    <div className='eps-update-card-main'>
        <div className='eps-update-card card'>
            <div className='card-body'>
              <div>
                <img onClick={returnWindow} src={arrow} className='eps-update-img' alt='img' />
              </div>
              <div className=' text-center'>
                <p className='eps-update-title'>Editar EPS</p>
              </div>
              <div className='mt-4'>
                <input value={inputName} onChange={(e) => setInputName(e.target.value)} type="text" className="eps-update-input form-control" placeholder="Nombre de la eps" />
              </div>
              <div className='mt-4'>
                {error && <p style={{ color: 'red' }}>{error}</p>}
              </div>
              <div className='mt-4 text-center'>
                <button onClick={edit} type="button" className="eps-update-button btn btn-primary">Guardar</button>
              </div>
            </div>
        </div>
    </div>
