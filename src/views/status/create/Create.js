...
function Create({ setView,getAll }) {
  ...
  const create = async () => {
    ...
    if(validate === true) {
      ...
      let response = await dispatch(createStatusService(body));
  ...
  return (
    <div className='status-create-card-main'>
        <div className='status-create-card card'>
            <div className='card-body'>
              <div>
                <img onClick={returnWindow} src={arrow} className='status-create-img' alt='img' />
              </div>
              ...
              <div className='mt-4 text-center'>
                <button onClick={create} type="button" className="status-create-button btn btn-primary">Guardar</button>
              </div>
            </div>
        </div>
    </div>
